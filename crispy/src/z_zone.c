...
// Z_Init
//
void Z_InitMem(byte* memory_zone, int size);

void Z_Init (void) {
    int size;
    byte* memory_block = I_ZoneBase(&size);
    Z_InitMem(memory_block, size);
}
...
void Z_InitMem(byte* memory_zone, int size)
{
    ...
    mainzone = (memzone_t *)memory_zone;
    mainzone->size = size;
    ...
    RecordHeapMetadata(&mainzone->blocklist, mainzone->size);
}
...
//          I_Error ("Z_Malloc: failed on allocation of %i bytes", size);
    ...
    RecordZMalloc(size, tag, user, result);
