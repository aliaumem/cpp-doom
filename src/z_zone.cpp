...
struct memblock_t {
    ...
    [[nodiscard]] bool isFree() const { return tag == purge_tags::PU_FREE; }
...
struct memzone_t {
        ...
        , rover { nullptr }
    {
        ...
        blocklist.prev = blocklist.next = rover = firstBlock;
    }
...
struct memory_resource {
    ...
    memory_resource(std::byte* memory_zone, size_t size)
    ...
    iterator begin() { return {mainzone->blocklist.next}; }
    iterator end() { return {&mainzone->blocklist}; }
    iterator rover() { return {mainzone->rover}; }
...
{
    for(auto* block : mem_res) //block->next != &mem_res->blocklist)
    {
        auto tag = block->tag;
        ...
        if (tag == PU_STATIC || tag == PU_LEVEL || tag == PU_LEVSPEC)
        {
            ...
            void** mem = reinterpret_cast<void **>(reinterpret_cast<byte *>(block) + sizeof(memblock_t));
...
void Z_Free (void* ptr)
{
    ...
    if (!block->isFree() && block->user != nullptr)
    {
    ...
    if (auto* previous = block->prev; previous->isFree())
    {
        // merge with previous free block
        previous->merge_with(block);

        if (mem_res->rover == block)
            mem_res->rover = previous;

        block = previous;
    }

    if (auto* next = block->next; next->isFree())
    {
        // merge the next free block onto the end
        block->merge_with(next);

        if (mem_res->rover == next)
            mem_res->rover = block;
    }
}
  ...
  void*		user )
{
    ...
    } while (!base->isFree() || base->size < size);
  ...
  int		hightag )
{
    ...
    for (block = mem_res->blocklist.next ;
	 block != &mem_res->blocklist ;
	 block = next)
    {
